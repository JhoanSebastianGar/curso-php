 ...
 * @param {lambda} fun
 */
confirm = (title, text, icon, confirmButtonText, fun, showCancelButton = true, cancelButtonText = "Cancelar") => {
    Swal.fire({
        title: title,
        text: text,
        icon: icon,
...
function validate(element) {
    if (isNaN(element)) {
        if (SELECTOR("#" + element).required == true && SELECTOR("#" + element).type !='submit' ) {
            if (SELECTOR("#" + element).value == '' && SELECTOR("#" + element).style.background != 'white') {
                SELECTOR("#" + element).style.background = "#FA8F8F";  
            }
         else {
            SELECTOR("#" + element).style.background = 'white';
            if (SELECTOR('input[name="error' + element + '"')) {
                SELECTOR('input[name="error' + element + '"').remove();
            }
        }
    }
        var div = document.getElementsByName("error" + element);
        div.forEach(function callback(currentValue, index) {
            if (index > 0)
                currentValue.remove();
        });
    }
...
/**
 * Obtiene el id desde el DOM option
 * Se require que tenga el atributo data-id en el DOM datalist
 * @param {string} value Valor a buscar
 * @param {datalist} datalist DOM datalist
 * @returns {int|string|bool} Retorna el ID; Sin embargo, en el caso de que no lo encuentre, retorna false
 */
function getIdFromOptionsDataList(value, datalist) {
    for (let i = 0; i < datalist.options.length; i++) {
        if (datalist.options[i].value == value) {
            return datalist.options[i].dataset.id;
        }
    }
    return null;
}